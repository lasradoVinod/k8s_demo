...
names=("go-client" "node-client" "java-client" "server")
...
for name in "${names[@]}"; do
    ...
    echo "[alt_names]
    DNS.1 = ${name}.default.svc
    " >> "/tmp/config.cnf"
    openssl req -new -key cert-config/$name-key.pem -out cert-config/$name.csr -config /tmp/config.cnf
    openssl x509 -req -in cert-config/$name.csr -CA cert-config/ca.pem -CAkey cert-config/ca-key.pem -CAcreateserial -out cert-config/$name-cert.pem -days 365 -sha256 -extfile /tmp/config.cnf
done
